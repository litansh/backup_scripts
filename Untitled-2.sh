...
function restore_replica_ () {
 rm -rf /var/www/devde01.tradenet.com/public_html/*
 ping 8.8.8.8 -c 10
 tar -zxvf /var/rep/www/tradenet.tar.gz -C /var/www/devde01.tradenet.com/public_html/
 ping 8.8.8.8 -c 10
 mysql -ureplica tradenet < /var/rep/sql/tradenet.com.sql
 ping 8.8.8.8 -c 10
 mv  /var/www/devde01.tradenet.com/public_html/var/www/tradenet.com/* /var/www/devde01.tradenet.com/public_html/
 ping 8.8.8.8 -c 10
 rm -rf /var/www/devde01.tradenet.com/public_html/var
 ping 8.8.8.8 -c 10
 rm -rf /var/www/devde01.tradenet.com/public_html/wp-config.php
 yes | cp -f /var/www/wp-config/devde01.tradenet.com/wp-config.php /var/www/devde01.tradenet.com/public_html/wp-config.php
 ping 8.8.8.8 -c 5
 echo 'Tradenet Replica is Done http://devde01.tradenet.com' | mail -s 'Tradenet Replica is Done' gennady@cglms.com
 ping 8.8.8.8 -c 5
}

function restore_sql_ () {
 # get today's date
 OUTPUT2="$( mysql -ureplica --verbose tradenet < /DEV-TO-PROD/tradenet.com/tradenet.com.sql)"
 OUTPUT1="$(date)"
 OUTPUT3="$(/var/www/bckp/sql.log)"
 OUTPUT="$(echo 'DEVDE01.tradenet.com WWW was restored http://devde01.tradenet.com' | mail -s 'DEVDE01.tradenet.com WWW was restored' it@cglms.com)"
 # You must add following two lines before
 # outputting data to the web browser from shell
 # script
 echo "Content-type: text/html"
 echo ""
 echo "<html><head><title>Restore</title></head><body>"
 echo "Today is $OUTPUT1 <br>"
 echo "Restore status is Done Enjoy!!!<br>"
 echo "</body></html>"
}

function check_dir_size_ () {
 gbsize=$"(du -hcs $WWWrepl/${dir[$i]} | grep GB | wc -l)"
 checksizemb="$(du -hcs $WWWrepl/${dir[$i]}  | awk '{print $1}' | head -n 1 | cut -d "M" -f 1)"
 checksizegb="$(du -hcs $WWWrepl/${dir[$i]}  | awk '{print $1}' | head -n 1 | cut -d "G" -f 1)"
 if [ $gbsize != "0" ] && [ $checksizegb > "2" ]; then
...
elif [ ${dir[$i]} != "" ] && [ $gbsize != "0" ] && [ $checksizegb > "2" ];
   mkdir -p $WWWrepl
   mkdir -p $WWWrepl/${dir[$i]}
...
GRANT EVENT, LOCK TABLES, RELOAD, REPLICATION CLIENT, SELECT, SHOW VIEW, TRIGGER on *.* to '$mysqlbackupuser'@'localhost';
FLUSH PRIVILEGES;
MYSQL_SCRIPT
  clear && echo -e "\n\n\n\t\t\t        ${bggrey}${black}${blink}$mysqlbackupuser password set to blank${normal}${menu} ${normal}" && sleep 5 && clear
  printf "\n\n\n\t\t\t        ${bggrey}${black}${blink}Created new user $mysqlbackupuser with correct backup permissions${normal}${menu} ${normal}"
  sleep 5
  clear
 }
...
function find_site_dir_ () {
 ...
 while [ -s /root/wwwdirtmp.txt ] && [ $compare == "ok" ]; 
  do
   comparison_  
   cat /root/wwwdirtmp.txt | cut -d"#" -f3- | tr " " "\n" | sed 's/^/#/' | tr " " "\n" > /root/wwwtmp.txt && mv -f /root/wwwtmp.txt wwwdirtmp.txt && rm -rf /root/wwwtmp.txt
  done
}
...
### Restore function
### 2GB
### if no sql -> do not run1
### Check restore on clone
### Clone prdweb
### Clone prddb
### 60.180 to DEBCKP
### Service
