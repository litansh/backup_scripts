...
then
 ...
 echo "$SSHADMIN" >> /usr/lib/cssam/backup/hosts/$SSHRIP/$SSHRIP.txt
...
function chk_box_manual_backup_ () {
 border_ssh_connected_hosts_ && clear
 all_sites_
 showusers=`cat /root/allsites.txt`
 USERLIST=`cat /root/allsites.txt `
 ...
 prompt=`echo -e "\n\t\tSelect hosts from list\n\t\tAgain to uncheck, ENTER when done:" `
 while menu_list_ && read -rp "$prompt" num && [[ "$num" ]]; do
 [[ "$num" != *[![:digit:]]* ]] && (( num > 0 && num <= ${#options[@]} )) ||
 {
 msg="Invalid option: $num"; continue
 }
 ((num--)); msg="${options[num]} was ${choices[num]:+un}checked" [[ "${choices[num]}" ]] && choices[num]="" ||
 choices[num]="[+]"
 done
 printf "\n\t\tYou selected: "; msg=" nothing"
 for i in ${!options[@]}; do [[ "${choices[i]}" ]] && { printf "\n\t\t %s" "${options[i]}"; msg=""; }
 done | tee >user

 for users in $SELECTEDU; do echo "Backing up "$users"" && sleep 4 && clear && backup_selected_ ; done
}
#
...
function Backup_Variables_ () {
  ...
  ROOTP="$(sed -n '4p' /usr/lib/cssam/backup/hosts/$users/$users.txt)"
  ...
  MYSQLR="$(sed -n '8p' /usr/lib/cssam/backup/hosts/$users/$users.txt)"
  MYSQLP="$(sed -n '9p' /usr/lib/cssam/backup/hosts/$users/$users.txt)"
 }
#
Backup_Variables_
check_backup_done_
...
function bckp_sql_ () {
 ...
 mysql -unewbckp -N -e 'show databases;' |
 while read dbname;
  do
   mkdir -p /root/DB
...
#
function exit_ () {
echo -e "${menu}Thank you ${number}"$whoami" ${menu}for use backup software${normal}" && sleep 4 && clear
exit 0
}
#
#*****************************#
...
if [ $x == 0 ];
 ...
 clear
  ...
  clear && echo -e "\n\n\n\t\t$SSHRIP has been added to backup!!" && logo_p_good_ && sleep 5
	return 0 && create_db_users_ && clear && start_menu_
...
GRANT EVENT, LOCK TABLES, RELOAD, sSELECT, SHOW VIEW, TRIGGER on *.* to '$mysqlbackupuser'@'localhost';
FLUSH PRIVILEGES;
MYSQL_SCRIPT
...
function bckp_sql_ () {
 ...
 while read dbname;
  do
   mkdir -p /root/DB/$dbname
   echo -e "Running sql backup on "$dbname".."
   mysqldump -unewbckpuser --complete-insert --routines --triggers --single-transaction "$dbname" > /root/DB/$dbname/"$dbname-$LD".sql;
   rm -rf performance*.sql && rm -rf mysql-*.sql
  done
EEE
EOF
...
#
function mail_done_ () {
 echo "Hi there Admin!" ;echo "" ;echo ""$HN" *"$IPL"* Backup done" | sed -e 's/^[ \t]*//' | mail -s "***Backup done on "$IPL" !!! "  -r " <$HN@$DN>" $RM
}
#
lines=`cd /usr/lib/cssam/backup/hosts && ls -A | wc -l`
...
while IFS= read -r line
 do
  ...
  function Backup_Variables_ () {
   ...
   ROOTP="$(sed -n '4p' /usr/lib/cssam/backup/hosts/$line/$line.txt)"
   WWWDIR="$(sed -n '5p' /usr/lib/cssam/backup/hosts/$line/$line.txt)"
   SSLDIR="$(sed -n '6p' /usr/lib/cssam/backup/hosts/$line/$line.txt)"
   SQLDIR="$(sed -n '7p' /usr/lib/cssam/backup/hosts/$line/$line.txt)"
   MYSQLR="$(sed -n '8p' /usr/lib/cssam/backup/hosts/$line/$line.txt)"
   MYSQLP="$(sed -n '9p' /usr/lib/cssam/backup/hosts/$line/$line.txt)"
  }
  #
  Backup_Variables_
  check_backup_done_
  ...
  if [ "$WWWDIR" != "" ]; then
   ...
   while IFS= read -r line
    do
     rsync -avzhe ssh -p "$IPPORT" root@$line:$WWWDIR/$line /usr/lib/cssam/backup/hosts/$line/WWW/TMP/$line/
     tar -cvzf /usr/lib/cssam/backup/hosts/$line/WWW/$line/$line_www${LD}.tar.gz /usr/lib/cssam/backup/hosts/$line/WWW/TMP/$line/;
   ...
   tar -cvzf /usr/lib/cssam/backup/hosts/$line/SSL/$line_ssl${LD}.tar.gz /usr/lib/cssam/backup/hosts/$line/SSL/TMP/;
   ...
   tar -cvzf /usr/lib/cssam/backup/hosts/$line/HTTPD/$line_httpd${LD}.tar.gz /usr/lib/cssam/backup/hosts/$line/HTTPD/TMP/;
  ...
  if [ "$SQLDIR" != "" ]; then
   ...
   sed -i "s/LINE/$IPADDR/g" /etc/backupmysql.sh
   sed -i "s/EEE/EOF/g" /etc/backupmysql.sh
   sed -i "s/EEEE/$endoffile/g" /etc/backupmysql.sh
   chmod +x /etc/backupmysql.sh
   /./etc/backupmysql.sh
   rm -rf /etc/backupmysql.sh
   mv /etc/backupmysqlORG.sh /etc/backupmysql.sh
   ...
   rsync -avzhe ssh -p "$IPPORT" $ROOTU@$line:/root/DB/* /usr/lib/cssam/backup/hosts/SQL/
  fi
  #
  function rm_old_bckp_ () {
   DELL=130
   DELLLESS=30
   CHK=`find /usr/lib/cssam/backup/hosts/SQL/ -mtime +$DEL | wc -l` > /dev/null 2>&1 && clear
   if [ "$CHK" != 0 ]; then
    find /usr/lib/cssam/backup/hosts/$line/SQL/*.* -mtime +$DEL -exec rm {} \;
