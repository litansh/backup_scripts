...
# function restore_from_bckp_timer_ () {
# run_script_ (){
# stty -echo && tput sc
# echo -ne "\033[0K\r"
# tput cuu1 && tput el
# echo -e "\n\n\t\t\tYou have $(($time-$count)) seconds to input 'r' and Enter or backing up"
# echo -n "$inputtimer"
# stty echo‏
# tput rc
# }
...
# clear
# inputkey="" && time=5
# trap exit_script_ SIGINT
# stty -icanon time 0 min 0 -echo
# echo -e "\n\t\t\tYou have $time seconds to input 'r' and Enter or backing up"
# while inputtimer=$inputtimer$inputkey; do
 # sleep 0.05
 # read inputkey && break
 # ((countdown  = countdown + 1 ))
 # if [[ countdown -eq 20 ]];then
  # ((count++))
  # countdown=0
  # run_script_ $inputtimer
 # fi
 # [[ $count -eq $time ]] && echo -e "\n\t\t\tYou are out of time.. backing up" && f_main
# done

# stty sane
 # if [ $inputtimer == "r" ]; then
  # inputtimer=""
  # restore_from_bckp_ 
 # else
  # f_main
...
# ‏function restore_from_bckp_ () {
 # while true;
  # do
   ...
   # clear && clear && clear
   # ccall_allert_title_restore_
   # echo -e "\n\t\t\t\t ${menu}${menu1}Restore Menu${normal}\n"
   # echo -e "\n\n\t\t\t${number}   Step 2${menu}: ${menu}Please enter date to restore from:${normal}"
   # local C2="$(printf "\t\t\t   ${bggrey}${black}${blink}===>${normal}${menu}: ${normal}")"
   # read -e -p "$C2" restoredate
   # clear && clear && clear
   # restore_site_
   # sleep 5
   # return 0 && break  
